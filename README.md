...
信息茧房是一个基于**橘鸦Juya**的B站AI早报制作的安全的**Edge/Chrome扩展**。

它不会把你的任何B站数据上传到服务器，也不会收集你的账号信息。

## 能做什么？

**把你认为不感兴趣的早报内容全自动跳过！** 我要是只想看OpenAI，Anthropic和xAI，那为什么我要在和我无关的内容上花时间？
...
- **没有评论索引时尝试 OCR**：从视频底部章节条切片、识别标题，再按当前关键词集合判断。只有命中关注关键词的片段会保留；必要时整页停止跳段。不过这一部分可能速度要慢一点。
...
2. 点击**加载解压缩的扩展**，选择刚克隆或解压的仓库文件夹`information-cocoon`，确认导入成功后启动扩展。
3. 打开橘鸦Juya的一期B站AI早报，先播放几秒让视频加载。点击工具栏上的橘鸦头像，选择关键词集合，打开**总开关**。
4. 等待插件自动下滑加载置顶评论区的播放索引，你也可以自己下滑。成功时，插件右上角会显示绿色的本页已启用。
5. 也支持Chrome浏览器。插件安装方法一致。
